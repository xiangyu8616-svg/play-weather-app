...
| 2.5 | 商店素材 | 部分完成 2026-08-05：英文上架文案 `docs/store-listing.md`（App Store 名称/副标题/关键词/描述 + Play 短描述）；英文截图 3 张已截（6.7" 1290×2796：首页 hero+拍摄窗口、预报页光线时间轴+7 天、我的页登录）存 `store-assets/`；待补：现象特写、3D 地球仪、6.5"（1242×2688）尺寸；已知瑕疵已全部修复（`83a174b`+`dc9cf01`）：英文城市名英化（cityNames.js 映射 18 城，首页/profile 接入，未命中回退原名）、风向/天气文本英化（windDirEn/weatherTextEn 防缓存竞态）、语言切换后自动重拉数据（prevLangRef）、Today/Tomorrow 折行/截断修复（dayName 宽 86 + numberOfLines=1），线上实测通过 |
