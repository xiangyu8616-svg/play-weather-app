...
| 2.6 | EAS Build 双端 profile | 配置就绪 2026-08-05：`eas.json`（preview-android APK/preview-ios/production 商店版 autoIncrement + appVersionSource remote）、`app.json` 补相册权限文案（App Store 审核必需）、分步操作指南 `docs/eas-build-guide.md`（账号前置/Android 封闭测试 14 天先行/iOS TestFlight/合规检查单）；待用户本人执行：eas login → eas init → eas build（详见指南） |
